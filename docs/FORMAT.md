...
默认封面使用 [logos.pdf](logos.pdf) 中从附件 3 提取的四个原始 Logo，并按原件设置学校、参赛队号和三位队员姓名。原件没有“选择题号”、独立年份或“参赛论文”标题，因此默认封面不印这些内容；`problem`、`year` 配置保留兼容。若希望直接使用在 Word 中填写完成的封面，也可按下文导入 A4 PDF。
...
两页的三行赛事标题由 LaTeX 文字排版，并在右下方偏移 `0.6pt` 叠加 `black!35` 灰色文字阴影。字形与阴影均随字体渲染，届次继续由 `edition` 配置控制；阴影不改变文字盒子的尺寸与行距。
...
`docs/logos.pdf` 随模板提供，四幅图像来自附件 3 转换后的 DOCX 首段内嵌资源，按原件的显示尺寸、顺序与裁剪方式排列：
...
LaTeX 以自然尺寸嵌入，西安交通大学图像按原件显示校徽，第二个 Logo 已处理 PDF 阅读器兼容性问题。Logo 直接取自本次附件，不重绘、不借用往届图形；其权利归相应权利人，模板的 MIT 许可不覆盖这些资源。`logos.pdf` 的 SHA-256 为 `10b275f772d02f1ac43349cf5137921b5a6f61e2b656d69accc46cee75d7866e`。

### 导入 Word 封面

如需直接导入填写后的 Word 封面，打开[附件 3](附件3：“华为杯”第二十三届中国研究生数学建模竞赛论文模板.doc)，填写学校、参赛队号和队员姓名，保留原版图形和布局，将封面导出为 A4 PDF，保存为 `docs/cover.pdf`。在 `config.local.tex` 中设置：

```tex
\gmcmsetup{cover-pdf={docs/cover.pdf}}
```

编译 `main.tex` 时，PDF 的第 1 页会按原尺寸替换默认封面，后续摘要从第 1 页编号。LaTeX 不会自动填写导入 PDF 中的个人信息；文件不存在时编译会报错。Word 转换可能在封面生成页码 0，导入前应去掉该页码。

`docs/cover.pdf` 已被 Git 忽略，避免将填写后的封面自动提交到公开仓库。手工打包 ZIP 时仍需自行检查个人信息。
...
目录默认显示一、二、三级标题，并收录参考文献、附录及附录小节。目录中的一级条目使用黑体，段前、段后间距均为 `0.5em`；相邻一级条目的间距不重复叠加。正文使用不带星号的标题命令，即可自动编号并进入目录：
...
正文七章均设置二、三级标题，三级编号形如 `1.1.1`、`7.3.1`。小节组织参考所提供的 2025 年获奖论文中的背景与任务、假设与符号、建模与求解、模型优缺点与推广等安排，并按本模板的简短示例设置具体标题；正式写作时可调整小节名称和数量。
